...
function parseEvent(event){
  return{
    ...
    timeStart: Number(new Date(event.start.utc)),
    timeEnd: Number(new Date(event.end.utc)),
...
function parseGnarlyJson(data, key){
  ...
  if(match){
    ...
    console.log('Could not match JSON')
    return null;
  }
}

function fetchNastyContent(options, callback){
  const { event, cache } = options;
  const url = event.url;
...
function fetchEvent(options, page, callback){
  ...
  const url = `https://www.eventbriteapi.com/v3/events/search/?location.within=50km&location.latitude=${config.lat}&location.longitude=${config.lon}&&categories=${topicCategories.tech}&token=${config.eventbrite.key}`;
  cache.request(url, config.cacheTime, function(error, data){
    ...
    async.mapLimit(events, 1, (data, doneMap) => {
      ...
      fetchNastyContent({event, cache}, (error, content) => {
