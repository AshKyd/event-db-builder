...
Database.prototype.addEvent = function (event, callback) {
  ...
  async.auto({
    organizers: done => this.addGeneric('organizers', organizerSchema, event.organizer, done),
    venue: (values, done) => {
      if (event.venue) return this.addGeneric('venues', venueSchema, event.venue, done);
      return done();
    },
    events: ['organizers', (values, done) => {
      event.organizerId = values.organizers.id;
